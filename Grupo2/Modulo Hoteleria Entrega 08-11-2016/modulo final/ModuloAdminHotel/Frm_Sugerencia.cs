...
{
    public partial class Frm_Sugerencia : Form
    {
        ...
        string NO_form = "15109";
        seguridad.bitacora bita = new seguridad.bitacora();



        public Frm_Sugerencia()
        {
            InitializeComponent();
        }


        public Frm_Sugerencia(DataGridView datagrid)
        {
            InitializeComponent();
        ...
        private void btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            txt_fecha.Text = date_Fecha.Value.ToString("yyyy-MM-dd");
            txt_estado.Text = cbo_estado.SelectedItem.ToString();
            txt_cliente.Text = cbo_nombre.SelectedValue.ToString();
            txt_empresa.Text = cbo_Empresa.SelectedValue.ToString();

            //try { 
            CapaNegocio fn = new CapaNegocio();
            TextBox[] textbox = { txt_tipo,txt_fecha,txt_Descripcion,txt_estado,txt_cliente,txt_empresa};
            ...
            else
            {
                ...
                if (editar)
                {
                    ...
                    bita.Modificar("Se modifico la tabla", "buzon");

                }
                else
                {
                    fn.insertar(datos, tabla);
                    bita.Insertar("Se inserto el registro ", "buzon");
                }
                fn.LimpiarComponentes(this);
                fn.ActualizarGrid(datagridantes, "select * from buzon", tabla);
        ...
        private void Frm_Sugerencia_Load(object sender, EventArgs e)
        {
            CapaNegocio fn = new CapaNegocio();
            fn.InhabilitarComponentes(this);
