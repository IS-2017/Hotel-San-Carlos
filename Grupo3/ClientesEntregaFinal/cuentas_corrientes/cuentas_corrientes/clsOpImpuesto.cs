...
namespace cuentas_corrientes
{
    class clsOpImpuesto
    {
        ...
        public static clsimpuesto Obtenerimp(int id_imp)
        {
            ...
            {
                pimp.icod = _reader.GetInt16(0);
                pimp.iporcentaje = _reader.GetDecimal(1);
                pimp.snombre = _reader.GetString(2);
                pimp.sdecripcion = _reader.GetString(3);
            }
        ...
        public static int Eliminar(clsimpuesto pimp )
        {
            int iretorno = 0;
            OdbcConnection conexion = seguridad.Conexion.ObtenerConexionODBC();
            OdbcCommand comando = new OdbcCommand(string.Format("update impuesto set estado='{0}' where id_impuesto_pk='{1}'",
                 pimp.snombre, pimp.icod), conexion);
            iretorno = comando.ExecuteNonQuery();
            conexion.Close();
