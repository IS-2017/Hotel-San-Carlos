...
namespace cuentas_corrientes
{
    public class clsOpParmFact
    {
        ...
        {
            int iretorno = 0;
            OdbcCommand comando = new OdbcCommand(string.Format("insert into parametros_fac (id_parametros_pk, empresa, id_impuesto) values (NULL,'{0}','{1}')",
               tp.icodmep, tp.codimp), seguridad.Conexion.ObtenerConexionODBC());
      ...
      /*  public static List<clsParamFact> Buscar(string nomtp) //Método tipo lista, que retornar el resultado dela busqueda
        ...
        public static clsParamFact ObtenerParmFact(int id_itp)
        {
            ...
            OdbcCommand _comando = new OdbcCommand(String.Format("select id_parametros_pk, empresa, id_impuesto from parametros_fac  where id_parametros_pk ='{0}' ", id_itp), conexion);
            OdbcDataReader _reader = _comando.ExecuteReader();
            while (_reader.Read())
            {
                ptp.icod = _reader.GetInt16(0);

                ptp.icodmep = _reader.GetInt16(1);
                ptp.codimp = _reader.GetInt16(2);
            }
        ...
        public static int Actualizar(clsParamFact ptp)
        {
            int iretorno = 0;
            OdbcConnection conexion = seguridad.Conexion.ObtenerConexionODBC();
            OdbcCommand comando = new OdbcCommand(string.Format("update parametros_fac set empresa='{0}', id_impuesto='{1}' where id_parametros_pk='{2}'",
             ptp.icodmep, ptp.codimp, ptp.icod), conexion);
        ...
        public static int Elimiar(clsParamFact ptp)
        {
            int iretorno = 0;
            OdbcConnection conexion = seguridad.Conexion.ObtenerConexionODBC();
            OdbcCommand comando = new OdbcCommand(string.Format("update parametros_fac set estado='desactivado' where id_parametros_pk='{0}'",
             ptp.icod), conexion);
            iretorno = comando.ExecuteNonQuery();
            conexion.Close();
