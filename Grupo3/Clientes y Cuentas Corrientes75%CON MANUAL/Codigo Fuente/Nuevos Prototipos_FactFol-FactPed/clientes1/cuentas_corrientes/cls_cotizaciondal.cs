...
namespace cuentas_corrientes
{
    public class cls_cotizaciondal
    {
        ...
        public static int Agregar(cls_cotizacion coti)  // procesode insertar un nueva cotizacion al sistema
        {
                ...
                int retorno = 0;
                int retorno2 = 0;
                OdbcCommand comando = new OdbcCommand(string.Format("Insert into cotizacion (nombre_cte, direccion_cte, apellido_cte, fecha_cot, nit_cte, estado_cot) values ('{0}','{1}','{2}', '{3}','{4}', '{5}')",
                coti.nombre_cte, coti.direccion_cte, coti.apellido_cte, coti.fecha_cot, coti.nit_cte, coti.estado_cot), Conexion.ObtenerConexionODBC());
...

        {
           ...
           "SELECT bien.id_bien_pk, bien.descripcion, precio.precio FROM bien, precio where bien.id_bien_pk = precio.id_bien_pk and bien.descripcion ='{0}'", descripcion), seguridad.Conexion.ObtenerConexionODBC());
...

        {
            ...
            int resultado = 0;
            OdbcConnection conexion = seguridad.Conexion.ObtenerConexionODBC();
            OdbcCommand comando = new OdbcCommand(string.Format(" Delete From detalle_cotizacion where detalle_cotizacion.id_cotizacion_pk = {0}", id_cotizacion), conexion);
            resultado++;
            if (resultado > 0)
            {
