...
namespace seguridad
{
    public class SistemaUsuarioDatos
    {
        ...
        public int InsertarUsuario2(string usuario,  string contraseña, string empleado, string empresa)
        {
            ...
            try
           {
                ...
                OdbcConnection con = Conexion.ObtenerConexionODBC();
                string query = "insert into usuario (usuario, contrasenia, fecha_creacion, id_empleados_pk, id_empresa_pk) values ('"+usuario+"', '" + contraseña + "', curdate(),'"+empleado+"', "+empresa+")";
                OdbcCommand cmd = new OdbcCommand(query, con);
                cmd.ExecuteNonQuery();
                con.Close();
                bitacora.Registro("Asignacion del usuario: " +usuario+ " al colaborador: " +empresa+ '-' +empleado, "usuario");
                return 1;
            }
            catch {
