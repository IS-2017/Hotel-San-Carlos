...
namespace produccion
{
    public partial class frm_nuevaformula : Form
    {
        ...
        private void btn_aceptar_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (row[0].ToString() != ""|| row[1].ToString()!="" ||row[2].ToString()!=""||row[3].ToString()!=""||row[4].ToString()!="")
                {
                    ...
                    string query = "insert into detalle_receta_mp(id_receta_pk,id_categoria_pk,id_bien_pk,cantidad,id_medida_pk,id_proceso_pk)values('" + ultimovalor + "','" + row[0].ToString() + "','" + row[1].ToString() + "','" + row[2].ToString() + "','lptm','" + row[4].ToString() + "')";
        ...
        {
            //string query="Select "
            FuncionesNavegador.CapaNegocio cn = new FuncionesNavegador.CapaNegocio();
            cn.Anterior(dgv_cuerpo_formula);
            TextBox[] textbox = { txt_cantidad };
            /*ComboBox[] combobox = { cmb_categoria, cmb_materia_prima, cmb_medida, cmb_proceso };
            cn.llenartextbox(textbox, dgv_cuerpo_formula);
            cn.llenarCbo(combobox, dgv_cuerpo_formula);*/
        }
