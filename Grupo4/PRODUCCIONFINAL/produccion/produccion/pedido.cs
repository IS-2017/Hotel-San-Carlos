...
namespace produccion
{
    public class pedido
    {
        ...
        public DataTable carga_dped(string id)
        {
            try
            {
                OdbcConnection con = seguridad.Conexion.ObtenerConexionODBC();
                OdbcCommand cmd = new OdbcCommand("Select p.fecha_entrega as fecha, p.hora_entrega as hora, concat(c.nombre,' ',c.apellido)as nombre from encabezado_pedido p, cliente c WHERE p.id_encabezado_pedido_pk='"+id+"' AND p.id_cliente_pk = c.id_cliente_pk", con);
                DataTable dt = new DataTable();
                OdbcDataAdapter da = new OdbcDataAdapter(cmd);
                da.Fill(dt);
                con.Close();
        ...
        public DataTable carga_detalle_pedido(string id)
        {
            try
            {
                OdbcConnection con = seguridad.Conexion.ObtenerConexionODBC();
                OdbcCommand cmd = new OdbcCommand("Select d.orden as Orden, d.cantidad as Cantidad, d.id_menu_pk , d.correlativo, concat(d.id_menu_pk,'-',d.correlativo)as ID, concat(m.nombre,'-',t.tamanio) as Nombre, d.descuento as Descuento, (p.precio*d.cantidad) as Importe from detalle_pedido_rest d, menu m, precio p, tamanio_porcion t WHERE id_encabezado_pedido_pk='" + id+"' AND m.id_menu_pk=d.id_menu_pk AND m.correlativo=d.correlativo AND m.id_precio =p.id_precio AND  p.id_tamaniop_pk = t.id_tamaniop_pk", con);
                DataTable dt = new DataTable();
                OdbcDataAdapter da = new OdbcDataAdapter(cmd);
                da.Fill(dt);
                con.Close();
        ...
        public int elimina_detalle(string id_enc, int orden)
        {
            try
            {
                OdbcConnection con = seguridad.Conexion.ObtenerConexionODBC();
                OdbcCommand cmd = new OdbcCommand("Delete from detalle_pedido_rest where id_encabezado_pedido_pk='"+id_enc+"' and orden="+orden+"", con);
                cmd.ExecuteNonQuery();
                con.Close();
                return 1;
            }
